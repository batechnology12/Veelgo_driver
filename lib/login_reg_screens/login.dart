...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void>_login() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        var response = await _apiProvider.login(email: email, password: password);

        // Dismiss the dialog before any navigation or snackbar display
        Get.back();
        if (response.statusCode == 200 || response.statusCode == 201) {
         ...
         Get.to(MainDashboardScreen());
          ...
          SnackbarUtils.showSnackbar(context, 'Login Failed'); // Show error message
        }
      } catch (e) {
      ...
      appBar: AppBar(
        ...
        title: RichText(
          text: TextSpan(
            ...
            children:  <TextSpan>[
              TextSpan(text: 'VEEL', style: inter1.copyWith(color: Colors.white)),
              TextSpan(text: 'GO', style: inter1.copyWith(color: Colors.yellowAccent)),
            ],
          ),
        ),
      ),
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
            ...
            children: [
               Padding(
                ...
                child: Text(
                  'Welcome Back',
                  style: inter1.copyWith(fontWeight: FontWeight.bold,fontSize: 22.sp,color: Colors.black),
                ),
              ),
               Padding(
                padding: const EdgeInsets.symmetric(horizontal: 10),
                child: Text('Enter your phone number and \nPassword',
                  style: inter1.copyWith(fontWeight: FontWeight.w700,fontSize: 13.sp,color: Colors.black),),
              ),
              Padding(
                padding: const EdgeInsets.symmetric(horizontal: 10,),
                child: Form(
                  ...
                  child: Column(
                    ...
                    children: [
                       Text(
                        'Phone Number*',
                         style: inter1.copyWith(fontWeight: FontWeight.w700,fontSize: 14.sp,color: Colors.blueGrey),
                      ),
                      ksize10,
                      TextFormAddedNumbr(
                     ...
                     ksize20,
                       Row(
                        ...
                        children: [
                           Text("Don't have an account ?",style: inter1.copyWith(fontSize: 15,fontWeight: FontWeight.w700),),
                           SizedBox(width: 3.w),
                          GestureDetector(
                            onTap: (){
